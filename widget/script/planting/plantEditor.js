...
apiready=function(){
    ...
    // api.ajax({
    //   url:pds.defaultUri + 'app/plant/queryDetails',
    ...
    //   if(ret){
        // if(data.farmId){
          ...
          //   script: 'detail('+api.pageParam.plantDetailId+')'
          // });
        // }else{
        //   api.execScript({
        //     name: 'plantEditor_win',
...
function detail(d){
  //alert(pds.defaultUri + 'app/plant/queryDetails')
    pds.ajaxSubmit({
    ...
    success:function(ret,err){
      ...
      $("#farmId").html('<option value='+data.farmId+' >'+data.farmName+'</option>')
      $("#landId").html('<option value='+data.landId+' >'+data.landName+'</option>')
      pds.ajaxSubmit({
               url:'app/farmland/findLandArea',
               type : "GET",
               data:{id:data.landId},
      ...
      if(ret.data.resourceInfos.length){
          ...
          //   script: 'data('+data.farmId+','+data.categoryId+','+data.processId+','+data.landId+')'
          // });
       }
     },
    error:function(e){
...
function data(id,crop,pro,land){
  pds.ajaxSubmit({
    url:'app/staff/farmland/get_farm_list',
    type : "GET",
    ...
    success:function(ret,err){
      ...
      pds.ajaxSubmit({
        url:'app/plant/catProcess',
        type : "GET",
        data:{catId:crop,landId:$("#landId").val()},
        success:function(ret,err){
          // api.hideProgress();
          //api.alert({msg:JSON.stringify(ret)});
          if(!ret.data.length){
 ...
 error:function(e){
  ...
  window.confirm = function (message) {
   ...
   function conf(){
       ...
       buriedPoint.pageId = 'page_refusersed_details_admin';
       buriedPoint.eventId = 'button_want_to_send_page_refusersed_details_admin';
       pds.ajaxSubmit({
   ...
   function save(){
     ...
     var buriedPoint = pds.buriedPoint();
     buriedPoint.pageId = 'page_refusersed_details_admin';
     buriedPoint.eventId = 'button_want_to_save_drafts_page_refusersed_details_admin';
     pds.ajaxSubmit({
     url:"app/buried_point/save",
     data:{"point":buriedPoint},
     success:function(v){
           ...
           success : function(e) {
             if(n==2){
               api.alert({
                 ...
                 var buriedPoint = pds.buriedPoint();
              	 buriedPoint.pageId = 'page_refusersed_details_admin';
              	 buriedPoint.eventId = 'button_send_page_refusersed_details_admin';
                	pds.ajaxSubmit({
                		url:"app/buried_point/save",
                		data:{"point":buriedPoint},
                		success:function(v){
             ...
             if(n==1){
               api.alert({
                 ...
                 var buriedPoint = pds.buriedPoint();
              	 buriedPoint.pageId = 'page_refusersed_details_admin';
              	 buriedPoint.eventId = 'button_save_drafts_page_refusersed_details_admin';
                	pds.ajaxSubmit({
                		url:"app/buried_point/save",
                		data:{"point":buriedPoint},
                		success:function(v){
